...
class InceptionTunerModelBase(KtModelBase):
    def model(self):
        ...
        for bl in range(self.hp.Int("num_blocks", **self.config.CONFIG_TRAINER["NUM_BLOCK"])):
    ...
    def model_block(self, input_, name):
        ...
        for i in range(self.hp.Int(self.wrap_name(name, "num_branches"), **self.config.CONFIG_TRAINER["NUM_BRANCHES"])):
        ...
        branch_last = self.get_conv_layer(input_=branch_last, name=b_last_name, f_name=b_last_f_name,
                                          ...
                                          kernel_size=1 if self.config.CONFIG_TRAINER["WITH_ONES"] else None)
    ...
    def model_branch(self, input_, name, idx):
        ...
        for idx_sub in range(1, 3):
            ...
            with_ones = self.config.CONFIG_TRAINER["WITH_ONES"]
...
class InceptionTunerModel3D(InceptionTunerModelBase):
    def get_conv_layer(self, input_, name, f_name, k_name, kernel_size=None):
        return layers.Conv3D(
            filters=self.hp.Int(f_name, **self.config.CONFIG_TRAINER["BLOCK_FILTERS"]),
            kernel_size=self.hp.Int(k_name, **self.config.CONFIG_TRAINER["BLOCK_KERNEL_SIZE"]) if kernel_size is None
            else kernel_size,
            padding="same",
            name=name)(input_)
