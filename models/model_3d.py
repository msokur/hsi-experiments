...
def inception3d_block(input_, factor=16):
    ...
    branch1 = tf.keras.layers.Conv3D(filters=factor * 4, kernel_size=1, padding="same", activation='relu',
                                     kernel_initializer=kernel_initializer,
                                     bias_initializer=bias_initializer)(input_)

    branch2 = tf.keras.layers.Conv3D(filters=factor * 6, kernel_size=1, padding="same", activation='relu',
                                     kernel_initializer=kernel_initializer,
                                     bias_initializer=bias_initializer)(input_)
    branch2 = tf.keras.layers.Conv3D(filters=factor * 8, kernel_size=3, padding="same", activation='relu',
                                     kernel_initializer=kernel_initializer,
                                     bias_initializer=bias_initializer)(branch2)

    branch3 = tf.keras.layers.Conv3D(filters=factor, kernel_size=1, padding="same", activation='relu',
                                     kernel_initializer=kernel_initializer,
                                     bias_initializer=bias_initializer)(input_)
    branch3 = tf.keras.layers.Conv3D(filters=factor * 2, kernel_size=5, padding="same", activation='relu',
                                     kernel_initializer=kernel_initializer,
                                     bias_initializer=bias_initializer)(branch3)

    branch4 = tf.keras.layers.MaxPooling3D(pool_size=3, strides=1, padding="same")(input_)
    branch4 = tf.keras.layers.Conv3D(filters=factor * 2, kernel_size=1, activation='relu',
                                     kernel_initializer=kernel_initializer,
