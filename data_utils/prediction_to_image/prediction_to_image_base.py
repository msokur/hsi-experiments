...
class PredictionToImage_base:
    def __init__(self, config, image_size=(480, 640)):
    ...
    def get_prediction_mask(self, spectrum_path: str, model_path: str) -> np.ndarray:
        ...
        model = self.load_model(model_path=model_path, custom_objects=self.CONFIG_TRAINER["CUSTOM_OBJECTS_LOAD"])
    ...
    def check_classes(self, predict_mask: np.ndarray) -> np.ndarray:
        ...
        if len(self.CONFIG_DATALOADER["MASK_COLOR"]) != len(self.CONFIG_DATALOADER["LABELS_TO_TRAIN"]):
            key_range = range(sorted(self.CONFIG_DATALOADER["MASK_COLOR"].keys())[-1])
            for label in key_range:
                if label not in self.CONFIG_DATALOADER["LABELS_TO_TRAIN"]:
    ...
    def mask_to_rgba(self, mask: np.ndarray, mask_alpha=1.0) -> np.ndarray:
        ...
        for label, color in self.CONFIG_DATALOADER["PLOT_COLORS"].items():
    ...
    def save_with_background(self, save_path: str, background_path: str, annotation_mask: np.ndarray,
        ...
        cube_ = cube(background_path, self.CONFIG_DATALOADER["WAVE_AREA"], self.CONFIG_DATALOADER["FIRST_NM"],
                     self.CONFIG_DATALOADER["LAST_NM"])
    ...
    def save_image(self, save_path: str, masks: list):
        ...
        for label_num in sorted(self.CONFIG_DATALOADER["LABELS_TO_TRAIN"]):
            cell_colors.append([self.CONFIG_DATALOADER["PLOT_COLORS"][label_num], "w", "w"])
            cell_text.append(["", f"{label_num}", self.CONFIG_DATALOADER["TISSUE_LABELS"][label_num]])
