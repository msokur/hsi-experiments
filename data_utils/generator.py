...
from configuration.keys import PathKeys as PK, CrossValidationKeys as CVK, TrainerKeys as TK, DataLoaderKeys as DLK, \
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self,
        ...
        self.batch_split = BatchSplit(labels_to_train=CONFIG_DATALOADER[DLK.LABELS_TO_TRAIN],
    ...
    def __getitem__(self, index):
        ...
        if CONFIG_TRAINER[TK.WITH_SAMPLE_WEIGHTS] and "weights" in data.keys():
    ...
    def get_valid_except_names(self):
        if len(self.valid_except_indexes) == 0:
            if CONFIG_CV[CVK.CHOOSE_EXCLUDED_VALID] == "restore":
                ...
                restore_paths = glob.glob(os.path.join(CONFIG_CV[CVK.RESTORE_VALID_PATH],
                                                       f"*{CONFIG_PATHS[PK.SYS_DELIMITER]}"))
                restore_path = restore_paths[np.flatnonzero(
                    np.core.defchararray.find(restore_paths, CONFIG_CV[CVK.RESTORE_VALID_SEQUENCE]) != -1)[0]]

                log_name = self.log_dir.split(CONFIG_PATHS[PK.SYS_DELIMITER])[-1]
                log_index = log_name.split("_")[1]  # can be problems

                restore_log_paths = glob.glob(os.path.join(restore_path, f"*{CONFIG_PATHS[PK.SYS_DELIMITER]}"))
                ...
                print(
                    ...
                    f"with {CONFIG_CV[CVK.RESTORE_VALID_SEQUENCE]}")
                return valid_except_indexes

            raw_paths = glob.glob(os.path.join(self.raw_npz_path, '*.npz'))
            raw_paths_names = [r.split(CONFIG_PATHS[PK.SYS_DELIMITER])[-1].split('.')[0] for r in raw_paths]

            print('Getting new validation patients')
            if CONFIG_CV[CVK.CHOOSE_EXCLUDED_VALID] == "randomly":
                return DataGenerator.get_random_choice(paths=raw_paths_names,
                                                       excepts=self.except_indexes,
                                                       size=CONFIG_CV[CVK.HOW_MANY_VALID_EXCLUDE])

            elif CONFIG_CV[CVK.CHOOSE_EXCLUDED_VALID] == "by_class":
    ...
    def choose_path(paths, paths_names, excepts, classes=None) -> ndarray:
        ...
        con_unique_classes = np.intersect1d(con_classes, CONFIG_DATALOADER[DLK.LABELS_TO_TRAIN])
        if len(con_unique_classes) >= len(CONFIG_DATALOADER[DLK.LABELS_TO_TRAIN]):
            return valid
    ...
    def get_class_weights(self, labels=None):
        ...
        for i, l in enumerate(labels):
            with np.errstate(divide="ignore", invalid="ignore"):
                weights[l] = (1 / sums[i]) * total / len(CONFIG_DATALOADER[DLK.LABELS_TO_TRAIN])
            if weights[l] == np.inf:
                weights[l] = 0.0
