...
class TFRSplit:
    ...
    def create_train_and_valid_tfrecord_files_II(self, archive_paths: List[str], out_files_dir: str,
                                                 train_names: List[str], valid_names: List[str]) -> Tuple[str, str]:
        train_out_file = self.__get_tfr_file(out_file_dir=out_files_dir, file_name=TRAIN)
        valid_out_file = self.__get_tfr_file(out_file_dir=out_files_dir, file_name=VALID)
        ...
        train_rest = {self.X_name: [], self.y_name: []}
        valid_rest = {self.X_name: [], self.y_name: []}
        ...
        for p in tqdm(sorted(archive_paths)):
            ...
            label_indexes = np.isin(y, self.use_labels)
            # ------------ get data indexes --------------
            train_data_indexes = label_indexes & np.isin(pat_names, train_names)
            self.__check_name_in_data(indexes=train_data_indexes, data_path=p, names=train_names)
            valid_data_indexes = label_indexes & np.isin(pat_names, valid_names)
            self.__check_name_in_data(indexes=valid_data_indexes, data_path=p, names=train_names)

            temp_train_rest = self.__split_and_save_batches__(writer_=train_writer, data=data,
                                                              data_indexes=train_data_indexes)
            temp_valid_rest = self.__split_and_save_batches__(writer_=valid_writer, data=data,
                                                              data_indexes=valid_data_indexes)

            for name in [self.X_name, self.y_name]:
                train_rest[name] += list(temp_train_rest[name])
                valid_rest[name] += list(temp_valid_rest[name])

        if len(train_rest[self.X_name]) >= 1000:
            self.__split_and_save_batches__(writer_=train_writer, data={k: np.array(v) for k, v in train_rest.items()},
                                            data_indexes=np.full(shape=len(train_rest[self.X_name]), fill_value=True))

        if len(valid_rest[self.X_name]) >= 1000:
            self.__split_and_save_batches__(writer_=valid_writer, data={k: np.array(v) for k, v in valid_rest.items()},
                                            data_indexes=np.full(shape=len(valid_rest[self.X_name]), fill_value=True))
    ...
    def __split_and_save_batches__(self, writer_, data, data_indexes: np.ndarray):
        # ---------------splitting into archives----------
        chunks = data_indexes.shape[0] // 1000
        chunks_max = chunks * 1000

        if chunks > 0:
            data_ = {k: np.array_split(data[k][...][data_indexes][:chunks_max], chunks) for k in
                     [self.X_name, self.y_name]}

            for row in range(chunks):
                features = get_features(X=data_[self.X_name][row], y=data_[self.y_name][row])
                examples = tf.train.Example(features=tf.train.Features(feature=features))
                writer_.write(examples.SerializeToString())

        # ---------------saving of the non equal last part for the future partition---------
        rest = {k: data[k][...][data_indexes][chunks_max:] for k in [self.X_name, self.y_name]}
        # ---------------saving of the non equal last part for the future partition---------
        return rest
