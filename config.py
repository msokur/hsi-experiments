...
#----------------------------------------------------------------------------------------------------------
RAW_NPZ_PATH = r'data_preprocessed/raw_3d_weighted'
...
WRITE_GRADIENTS_EVERY_Xth_BATCH = 50000000000 # callbacks #every GRADIENTS_WRITING_STEP batches we write gradients, so not epochs - gradients
...
if MODE == 'SERVER' or MODE == 'NO_GPU':
    preffix = r'/work/users/mi186veva/'

    RAW_NPZ_PATH = os.path.join(preffix, RAW_NPZ_PATH)
    SHUFFLED_PATH = os.path.join(preffix, SHUFFLED_PATH)
    BATCHED_PATH = os.path.join(preffix, BATCHED_PATH)
