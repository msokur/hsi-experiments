...
def test_func_one(fix_arg: int, parallel_arg: int) -> int:
    return parallel_arg * fix_arg


def test_func_two(fix_arg: int, parallel_arg_one: int, parallel_arg_two: int) -> float:
    return parallel_arg_one / parallel_arg_two * fix_arg


def test_func_three(fix_arg_one: int, fix_arg_two: int, parallel_arg: int) -> float:
    return parallel_arg * fix_arg_one / fix_arg_two


def test_func_four(fix_arg_one: int, fix_arg_two: int, parallel_arg_one: int, parallel_arg_two: int) -> float:
...
POOL_DATA = [(test_func_one, [PARALLEL_ARG_ONE], [FIX_ARG_ONE], [2, 4, 6, 8, 10, 12, 14, 16]),
