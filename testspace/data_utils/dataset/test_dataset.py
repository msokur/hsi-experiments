...
def get_test_datasets(test_config, data_typ: str, shape: str, with_sw: bool, file_dir: str, batch_size=5):
    ...
    if data_typ == "tfr":
        ...
        datasets = GeneratorDatasets(config=test_config, data_storage=npz_storage)
        paths = npz_storage.get_paths(storage_path=sh_path)
    else:
        zarr_storage = DataStorageZARR()
        datasets = GeneratorDatasets(config=test_config, data_storage=zarr_storage)
        paths = zarr_storage.get_paths(storage_path=sh_path)

    return datasets.get_datasets(dataset_paths=paths, train_names=USE_NAMES, valid_names=[], labels=LABELS,
...
def test_get_datasets_value(_delete_batches, test_config, data_dir: str, data_type: str, shape: str, with_sw: bool,
    ...
    dataset = get_test_datasets(test_config=test_config, data_typ=data_type, shape=shape, with_sw=with_sw,
                                file_dir=data_dir, batch_size=batch_size)[0]
