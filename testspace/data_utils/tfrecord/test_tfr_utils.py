...
from data_utils.tfrecord.tfr_utils import cw_from_meta, parse_names_to_int
...
def test_cw_from_meta(tfr_data_dir: str, shape: str, labels: list, names: list, result: dict):
    stored_path = os.path.join(tfr_data_dir, shape)
    cw = cw_from_meta(stored_path=stored_path, labels=labels, names=names)
...
def test_cw_from_meta_error():
    with pytest.raises(ValueError, match="No meta files found in the directory 'test'!"):
        cw_from_meta(stored_path="test", labels=[], names=[])


def test_parse_names_to_int(tfr_1d_data_dir: str):
    names_int = parse_names_to_int(stored_path=tfr_1d_data_dir)
...
def test_parse_names_to_int_error(tfr_data_dir: str):
    with pytest.raises(ValueError, match="Too many patient indexes in meta files for the name 'test_4'!"):
        names_int = parse_names_to_int(stored_path=os.path.join(tfr_data_dir, "meta_files_error"))
