...
GET_SPLITS_DATA = [("Name", PATHS, (0, 5), [np.array([0, 2]), np.array([1, 3])]),
                   ...
                   ("Files", PATHS, 2, [np.array([0, 1]), np.array([2, 3])])]
...
def test_get_splits_warning(typ, values, message):
    ...
    result = [np.array([0]), np.array([1]), np.array([2]), np.array([3])]
...
def test_split_list_error():
    with pytest.raises(ValueError, match="More than one match with the name 'data0'. Check your split name list!"):
        split_list(paths=PATHS, name_list=[["data0"], ["data1_1", "data1_2"]])


SPLIT_NAME_DATA = [(PATHS, (0, 5), [np.array([0, 2]), np.array([1, 3])]),
                   (PATHS, (0, 15), [np.array([0]), np.array([2]), np.array([1]), np.array([3])]),
                   ([], (0, 5), [])]
