...
WEIGHT_GET_OR_SAVE_DATA = [(WeightsNPZ, "npz_file", "1d"),
                           (WeightsZARR, "zarr_file", "1d")]


@pytest.mark.parametrize("weight_calc,archive_path,folder", WEIGHT_GET_OR_SAVE_DATA)
def test_weights_get_or_save_only_get(data_dir: str, weight_calc, archive_path: str, folder: str):
    weights_class = weight_calc(filename=FILENAME, labels=LABELS)
    weights = weights_class.weights_get_or_save(root_path=os.path.join(data_dir, archive_path, folder))
    os.remove(os.path.join(data_dir, archive_path, folder, FILENAME))
    assert (weights == WEIGHTS_RESULT).all()


@pytest.mark.parametrize("weight_calc,archive_path,folder", WEIGHT_GET_OR_SAVE_DATA)
def test_weights_get_or_save_only_save(data_dir: str, weight_calc, archive_path: str, folder: str):
    main_path = os.path.join(data_dir, archive_path, folder)
    weight_calc(filename=FILENAME, labels=LABELS).weights_get_or_save(root_path=main_path)
