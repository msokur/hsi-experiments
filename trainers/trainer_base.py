...
class Trainer:
    def __init__(self, config, model_name: str, except_indexes=None, valid_except_indexes=None):
        self.config = config
    ...
    def logging_and_copying(self):
        if self.config.CONFIG_TRAINER["TYPE"] != "Restore":
    ...
    def get_datasets(self, for_tuning=False):
        ...
        self.batch_path = self.config.CONFIG_PATHS["BATCHED_PATH"]
        ...
        train_generator = generator.DataGenerator(self.config,
                                                  "train",
                                                  self.config.CONFIG_PATHS["SHUFFLED_PATH"],
                                                  self.batch_path,
                                                  batch_size=self.config.CONFIG_TRAINER["BATCH_SIZE"],
                                                  split_factor=self.config.CONFIG_TRAINER["SPLIT_FACTOR"],
        ...
        valid_generator = generator.DataGenerator(self.config,
                                                  "valid",
                                                  self.config.CONFIG_PATHS["SHUFFLED_PATH"],
                                                  self.batch_path,
                                                  batch_size=self.config.CONFIG_TRAINER["BATCH_SIZE"],
    ...
    def get_callbacks(self):
        ...
        checkpoints_callback = keras.callbacks.ModelCheckpoint(
            ...
            monitor=self.config.CONFIG_TRAINER["MODEL_CHECKPOINT"]["monitor"],
            verbose=1,
            save_best_only=self.config.CONFIG_TRAINER["MODEL_CHECKPOINT"]["save_best_only"],
            mode=self.config.CONFIG_TRAINER["MODEL_CHECKPOINT"]["mode"])
        ...
        if self.config.CONFIG_TRAINER["EARLY_STOPPING"]["enable"]:
            early_stopping_callback = keras.callbacks.EarlyStopping(
                monitor=self.config.CONFIG_TRAINER["EARLY_STOPPING"]["monitor"],
                mode=self.config.CONFIG_TRAINER["EARLY_STOPPING"]["mode"],
                min_delta=self.config.CONFIG_TRAINER["EARLY_STOPPING"]["min_delta"],
                patience=self.config.CONFIG_TRAINER["EARLY_STOPPING"]["patience"],
                verbose=1,
                restore_best_weights=self.config.CONFIG_TRAINER["EARLY_STOPPING"]["restore_best_weights"])
    ...
    def train_process(self, mirrored_strategy=None):
        ...
        train_dataset, valid_dataset, train_generator, class_weights = self.get_datasets(
            for_tuning=self.config.CONFIG_TRAINER["SMALLER_DATASET"])
        ...
        if self.config.CONFIG_CV["MODE"] == 'RUN':
    ...
    def train(self):
        try:
            if self.config.CONFIG_PATHS["MODE"] == 'LOCAL_GPU' or self.config.CONFIG_PATHS["MODE"] == 'CLUSTER':
        ...
        if not self.config.CONFIG_TRAINER["EARLY_STOPPING"]["enable"]:
    ...
    def get_output_shape(self):
        if "OUTPUT_SIGNATURE_X_FEATURES" in self.config.CONFIG_DATALOADER:
            shape_spec = self.config.CONFIG_DATALOADER["OUTPUT_SIGNATURE_X_FEATURES"]
        else:
            shape_spec = self.config.CONFIG_DATALOADER["LAST_NM"] - self.config.CONFIG_DATALOADER["FIRST_NM"]

        if self.config.CONFIG_DATALOADER["3D"]:
            output_shape = (self.config.CONFIG_DATALOADER["3D_SIZE"][0],
                            self.config.CONFIG_DATALOADER["3D_SIZE"][1], shape_spec)
        else:
