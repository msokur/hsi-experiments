    ...
    def stack_columns(self, data, suitable_columns):
        ...
        for column in suitable_columns:
            ...
            for row_i, row in enumerate(rows):
                if not convertible_to_float(row):
                    row = row.replace('[', '')
                    row = row.replace(']', '')
                    row = row.split(' ')
...
if __name__ == '__main__':
    ...
    # reader = MetricsCsvReaderComparisonFiles() reader.read_metrics(
    # '/home/sc.uni-leipzig.de/mi186veva/hsi-experiments/metrics/Esophagus_MedFilter/cp-0000/compare_all_thresholds
    # .csv', names=['Sensitivity', 'Specificity'])
